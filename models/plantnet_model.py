...
class PlantNetModel(BasePlantModel):
    def __init__(self):
        ...
        self.class_mapping_path = os.path.join(self.model_dir, "plantnet300k_class_mapping.json")
    ...
    def load(self, model_path=None):
        ...
        if not target_path:
            possible_files = [f for f in os.listdir(self.model_dir) if f.endswith(('.pth', '.tar', '.pt'))]
            if possible_files:
        ...
        id_to_name_path = os.path.join(self.model_dir, "plantnet300K_species_id_2_name.json")
        ...
        if os.path.exists(idx_to_id_path) and os.path.exists(id_to_name_path):
            ...
            for idx, spec_id in idx_to_species_id.items():
                ...
                name = species_id_to_name.get(str(spec_id)) or species_id_to_name.get(spec_id)
                self.classes[str(idx)] = name if name else f"Unknown_Species_{spec_id}"

            print(f"[{self.name}] Successfully loaded {len(self.classes)} mapped species names.")
        else:
            print(f"[{self.name}] Warning: Mapping JSON files not found in {self.model_dir}. Using fallback indices.")
    ...
    def predict(self, image_path):
        ...
        check_low_confidence_alternatives(
            self.name,
            probs,
            lambda idx: self.classes.get(str(idx), f"Index_{idx}")
        )
